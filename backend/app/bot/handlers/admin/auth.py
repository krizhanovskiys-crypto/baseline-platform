...
async def cmd_dev(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    if role is None:
        # Not an operator. /dev is Admin/Owner only (Sprint 12.2) —
        # Verified Coach reaches Tournament features from the Main
        # Menu's own 🏆 Tournaments button, never here. Everyone else
        # sees nothing, exactly as if /dev does not exist.
        return
